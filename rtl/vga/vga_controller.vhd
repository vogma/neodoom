...
ENTITY vga_controller IS
    PORT (
        ...
        --   FIFO   --
        empty : IN STD_LOGIC;
        pxl_data : IN STD_LOGIC_VECTOR(31 DOWNTO 0);
        rd_en : OUT STD_LOGIC;
...
ARCHITECTURE rtl OF vga_controller IS
    ...
    -- because DOOM is rendering at 640*400, we have 40 black lines at the top and bottom to center the frame
    CONSTANT LETTERBOX_TOP : NATURAL := 40;
    CONSTANT GAME_HEIGHT : NATURAL := 400;
    CONSTANT GAME_BOTTOM : NATURAL := GAME_HEIGHT + LETTERBOX_TOP; -- lowest line of game frame
    ...
    SIGNAL in_game_region : STD_LOGIC := '0';
BEGIN
    ...
    in_game_region <= '1' WHEN vsync_reg >= LETTERBOX_TOP AND vsync_reg < GAME_BOTTOM ELSE
        '0';

    in_drain_window <= '1' WHEN vsync_reg >= GAME_BOTTOM AND vsync_reg < GAME_BOTTOM + DRAIN_LINES_C ELSE
        '0';

    -- Horizontal counter
    line_finished <= '1' WHEN hsync_reg = H_TOTAL_LINE - 1 ELSE
        '0';
    hsync_next <= 0 WHEN hsync_reg = H_TOTAL_LINE - 1 ELSE
        hsync_reg + 1;
    ...
    -- Active video region
    in_active_video <= '1' WHEN hsync_reg < FRAME_WIDTH AND vsync_reg < FRAME_HEIGHT ELSE
        '0';
    in_vblank <= '1' WHEN vsync_reg >= FRAME_HEIGHT ELSE
        '0';
    -- in_drain_window <= '1' WHEN vsync_reg >= FRAME_HEIGHT AND vsync_reg < FRAME_HEIGHT + DRAIN_LINES_C ELSE
    --     '0';
    ...
    rd_en_int <= '1' WHEN rst = '1'
        AND rd_rst_busy_i = '0'
        AND empty = '0'
        AND hsync_reg < FRAME_WIDTH
        AND (in_game_region = '1' OR in_drain_window = '1')
        ELSE
        '0';

    -- RGB888 -> RGB444
    vga_r <= pxl_data(23 DOWNTO 20) WHEN in_active_video = '1' AND in_game_region = '1' AND empty = '0' AND rst = '1' ELSE
        "0000";
    vga_g <= pxl_data(15 DOWNTO 12) WHEN in_active_video = '1' AND in_game_region = '1' AND empty = '0' AND rst = '1' ELSE
        "0000";
    vga_b <= pxl_data(7 DOWNTO 4) WHEN in_active_video = '1' AND in_game_region = '1' AND empty = '0' AND rst = '1' ELSE
        "0000";
    ...
    BEGIN
        IF rising_edge(pxl_clk) THEN
            ...
            ELSE
                ...
                IF in_active_video = '1' AND in_game_region = '1' AND empty = '1' AND rd_rst_busy_i = '0' THEN
                    underrun_sticky <= '1';
                END IF;
            END IF;
        END IF;
